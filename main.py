...
# Coloque as suas chaves do Supabase aqui
...
def descobrir_estrutura(df_raw: pd.DataFrame):
    ...
    if header_idx is None:
        ...
        # Nenhuma keyword exata encontrada — não arriscamos um fallback

        # numérico fixo (isso é justamente o que causava os bugs antigos).
    ...
    # Garante nomes de coluna únicos (planilhas podem repetir rótulos)
...
# 2. Seleção de colunas (setor / métrica) — match exato antes de substring
...
def encontrar_coluna_setor(colunas) -> str:
    ...
    # 2º: substring, mas NUNCA em colunas de data (ex.: "Data Admissão Setor")
...
def detectar_origem_e_coluna_valor(colunas) -> Optional[tuple]:
    ...
    """Decide se o arquivo é INTERNADOS ou ATENDIDOS e escolhe a coluna de

    valor correta, com prioridade explícita para evitar contar a métrica

    errada (ex.: tempo total de internação em vez de tempo no setor)."""
    ...
    if tem_permanencia or (tem_dia and not tem_realizado):
        ...
        # prioridade 1: permanência especificamente NO SETOR
        ...
        # prioridade 2: qualquer permanência
        ...
        # prioridade 3: coluna com "dia(s)" como palavra
    ...
    if tem_realizado:
        ...
        # prioridade 2: qualquer "realizado"
...
# 3. Filtro anti-lixo (regex de verdade, não apenas "não bateu no De-Para")
...
async def processar_pep(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        for file in files:
            ...
            if deteccao is None:
                ...
                avisos.append(
                    ...
                    "relatório de Internados ou Atendidos (colunas de "

                    "PERMANÊNCIA/DIA/REALIZADO não encontradas)."
        ...
        return {
            ...
            "avisos": avisos,  # extra, não quebra o contrato do front-end
